...
| [process.md](process.md) | 20 | pre-write Glob sweep, JSON/YAML validate-at-write, prior-phase re-verification, Windows Write over mkdir, .gitkeep vs README, lessons-learned cadence, diagnostic playbook (read transcript, one-sentence hypothesis, ground-truth tool, quantify, bounded substitution, discovery-as-regression-test, cross-class verify, scope expansion), peer-code as cross-domain learning vector, CF premise discipline, proof-of-concept-first development |
