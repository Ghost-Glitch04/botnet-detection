...
| powershell,compatibility,dns | Resolve-DnsName -QuickTimeout is not reliably available in PS 5.1 -- use Resolve-DnsName -Type TXT -DnsOnly without the flag; -NoNetworkLookups is the correct operator tool for blocked/slow DNS environments | phase17_static_analysis_and_ps51_validation.md#bug1 | pitfall |
| process,auditing,portability | Permanent regression test scripts committed to the repo must not contain machine-specific paths -- use $PSScriptRoot-relative paths; a grep for the author's username at pre-commit time catches this class | phase17_static_analysis_and_ps51_validation.md#bug2 | pitfall |
| testing,verification,ps51 | PS 5.1 Add-Member -Force + ConvertTo-Json produces identical JSON schema to PS 7 -- ASN field shape (RemoteASN/ASName/Country/CIDR) confirmed on PSVersion=5.1.26100.7920 via Tier 7 cross-host run | phase17_static_analysis_and_ps51_validation.md#cf42-closure | went-well |
| enrichment,caching,network | IP cache efficiency scales with traffic concentration -- 47 rows to 6 unique IPs on a clean VM (87% cache hit rate); design correctly matched to real-world traffic patterns where CDN/cloud ASNs dominate | phase17_static_analysis_and_ps51_validation.md#notable-observations | went-well |
| process,auditing | Static analysis top-down + bottom-up in parallel surfaces different bug classes -- top-down catches control-flow issues, bottom-up catches portability and compatibility bugs that look syntactically valid | phase17_static_analysis_and_ps51_validation.md | design |
